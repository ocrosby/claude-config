...
Source: adapted from [fredrikaverpil/dotfiles](https://github.com/fredrikaverpil/dotfiles/blob/main/stow/shared/.claude/skills/neovim/SKILL.md).
...
The plugin manager [lazy.nvim](https://github.com/folke/lazy.nvim) uses its own directory layout, separate from Neovim's built-in `pack/` structure.

### Plugin install directory

Plugins are installed under `stdpath("data")/lazy/` (e.g. `~/.local/share/nvim/lazy/<plugin-name>/`, or `~/.local/share/nvim-<appname>/lazy/<plugin-name>/` when `NVIM_APPNAME` is set). This path is **not** part of the standard Neovim `packpath`.
...
Plugins with `dev = true` in their spec are loaded from a local development path instead of the install directory.
...
3. **Verify independently** — if diagnostics seem wrong, run the linter directly to confirm the actual state (e.g. `golangci-lint run ./...` for Go, `make lint` for Lua/yoda.nvim).

## Safety

- **Never** send `:q`, `:qa`, `:bdelete`, or other destructive commands without explicit user confirmation.
- **Never** modify buffer contents via RPC without asking first — the user may have unsaved work or an undo history they care about.
- Prefer `--remote-expr` (read-only queries) over `--remote-send` (simulates typing) whenever possible.
- Always use command substitution + `grep -v` to suppress the `NVIM_APPNAME` warning (see Prerequisites).
