...
**Success criteria**: Contributor rankings (all-time and recent), lines-changed breakdown, per-subsystem ownership, commit samples, and (if GitHub is available) review patterns are produced, with a nuanced bus-factor assessment that goes beyond raw commit counts.
...
**Success criteria**: A ranked list of bug-hotspot files is produced, enriched with issue data if GitHub is available.
...
**Success criteria**: A monthly commit-count timeline is produced with trend observations, enriched with PR merge and release cadence if GitHub is available.
...
**Success criteria**: A count and list of firefighting commits (and PRs if GitHub is available) is produced.
