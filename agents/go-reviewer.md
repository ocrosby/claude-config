...
> **Standards reference**: Your review criteria align with `go-conventions.md` and `go-testing.md`. When the checklist below and those rules diverge, the rules are the source of truth. When the code under review touches auth, input parsing, deserialization, secrets, or network I/O, also load `rules/owasp-top-10.md` and apply its recognition signals.
