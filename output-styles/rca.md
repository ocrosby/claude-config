...
**Prevention**
The rule, hook, test, or skill that would catch this next time. If none exists, propose one and name the file it would live in — for example, "Add a hook at `hooks/<name>.sh` that fails CI when X" or "Add a finding to `rules/<name>.md` requiring Y." If the incident is a candidate for the README learnings log (per `rules/readme-learnings.md`), say so.
